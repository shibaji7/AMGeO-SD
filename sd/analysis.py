...
class Simulate(object):
    ...
    def __init__(self, rad, date_range, scan_prop, verbose=False, _dict_={}, cores=16):
        ...
        self.atrs = self.update_vals(scan_prop)
        ...
        if hasattr(self, "save") and self.save: self.to_netcdf()
        return
    
    def update_vals(self, scan_prop):
        atrs = {}
        atrs.update(self.__dict__)
        atrs.update(scan_prop)
        atrs.update({"type":"auroral", "gflg_type":"us_m", 
                     "gflg_type_desc":"Generated by Median Filtering algo."})
        for k in atrs.keys():
    ...
    def to_netcdf(self):
        bmax = 0
        for s in self.fscans:
            bmax = len(s.beams) if bmax < len(s.beams) else bmax
        fov = utils.geolocate_radar_fov(self.rad, [s.stime for s in self.fscans], azm=True)
        newkeys = [{"key":"gflg_conv", "long_name":"Ground scatter flag", "type": bool,
                    "desc":"Ground scatter flag (determined using median filter Conventional Method) (0/1)"},
                   {"key":"gflg_kde", "long_name":"Ground scatter flag", "type": bool,
                    "desc":"Ground scatter flag (determined using median filter KDE) (0/1)"},
                   {"key":"v_mad", "long_name":"Velocity median absolute deviation (MAD)", "type": float,
                    "desc":"Median absolute deviation after applying median filter."}]
        ds = utils.to_xarray(self.fscans, fov, bmax, self.atrs, newkeys)
        print(ds)
...
#             cm._plot_med_filt()
    ...
    def _fetch(self):
        ...
        d = self.io.scans_to_pandas(scans)
