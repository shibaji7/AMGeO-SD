...
ax.scatter(rand_jitter(df.slist)[sais], rand_jitter(v)[sais], c="r", s=1, label="IS + SAIS + Others")
ax.scatter(rand_jitter(df.slist)[~sais], rand_jitter(v)[~sais], c="k", s=1, label="GS")
ax.set_ylim(-vlim,vlim)
ax.legend(scatterpoints=3, ncol=1, fontsize=10, frameon=True)
ax.grid(True)
ax.set_ylabel("Velocity [m/s]")
